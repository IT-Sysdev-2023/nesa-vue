...
const records = ref<Product | null>(null);
...
const getProductData = async () => {
    ...
    try {
        const { data } = await axios.get(route("admin.get.product"));
...
onMounted(() => {
    ...
    window.Echo.private(`syncing-products.${page.auth.user.id}`).listen(
